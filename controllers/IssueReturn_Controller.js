...
const IssueBook = (req, res) => {
    ...
    db.serialize(() => {
        ...
        db.get(
            `SELECT * FROM ` + req.body["role_search"] + ` WHERE roll_number = ?`, [req.body["roll_number"]],
            (err, result) => {
                    ...
                    db.all(
                        ...
                        (err, book_titles) => {
                            if (book_titles.length == req.body["book_ids"].length) {
                                ...
                                res.send(
                                    JSON.stringify({
                                        message: "Book Successfully Issued to RollNumber : " +
                                            req.body["roll_number"],
                                    })
                                );
                            } else {
...
const ShowIssuedCheck = (req, res) => {
    db.get(
        `SELECT * FROM ` + req.body["roll_search"] + ` WHERE roll_number = ?;`, [req.body["roll_number"]],
        (err, detail_result) => {
            if (detail_result == undefined) {
                ...
                db.all(
                    `SELECT * FROM Book_Issued WHERE roll_number = ?`, [req.body["roll_number"]],
                    (err, issued_result) => {
                        if (issued_result.length == 0) {
                            res.send(
                                JSON.stringify({
                                    message: "No Book Issued to Roll Number : " +
                                        req.body["roll_number"],
    ...
    db.get(
        ...
        (err, detail_result) => {
            if (detail_result == undefined) {
                ...
                db.all(
                    `SELECT * FROM Book_Issued WHERE roll_number = ? AND role = ?`, [req.body["roll_number"], req.body["role"]],
                    (err, return_result) => {
